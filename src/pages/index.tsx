...
export default function Home() {
  ...
  const { messages: toastMessages, removeToast } = useToast();
  ...
  const handleSendChat = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        const stream = await getChatResponseStream(processedMessages, "", "");
        
        if (!stream) {
          handleApiError(new Error('获取AI响应失败'));
        ...
        while (true) {
          ...
          if (sentenceMatch && sentenceMatch[0]) {
            ...
            const trimmedSentence = sentence.replace(
              /^[\s\[\(\{「［（【『〈《〔｛«‹〘〚〛〙›»〕》〉』】）］」\}\)\]]+$/g,
              ""
            );
        ...
        handleApiError(error, '聊天发送失败');
      } finally {
        setChatProcessing(false);
      }
    },
    [systemPrompt, chatLog, handleSpeakAi, selectedVoiceId, characterName]
  );
  ...
  return (
    <div className={`${m_plus_2.variable} ${montserrat.variable}`}>
      ...
      <ErrorBoundary>
        ...
        <button
          onClick={toggleTheme}
          className="absolute top-4 right-20 z-10 w-10 h-10 rounded-full bg-surface1 hover:bg-surface1-hover border border-white/10 flex items-center justify-center text-text-secondary hover:text-text-primary transition-all text-lg md:w-12 md:h-12"
